...
###K-Nearest Neighbor Simulator (KNN)###

#Code for implementing KNN seperately to each site individually.  
#This code will be able to do one at a time only. 
...
###Output
#   1. A single KNN Simulation Realization

knn <- function(Fld,ngrids, N_valid, #Data Parameters
...
{ 
  ...
  for (i in (max_embd+1):N_valid){
    ...
    for (j in 1:ngrids){
      ...
      nn <- knn.sim.index(X_t[,,j],xtest,nneib,weights)
      nn_index <- sample(nn$yknn,1,prob = pj)
      
      Xnew[i,j] <- Y_t[nn_index,j]
    }
  }
  n_site <- ngrids/2
  WPnew = Xnew[,1:n_site]
