...
get_pca_pacf <- function(Fld, Field_Name){
  ...
  acf(pcw$x[,1], main = paste0("ACF for Reanalysis Data PC-1 - ",Field_Name),
      xlab = "Lag (Days)",
      xlim = c(0,30), ylim = c(-0.05,1),
      lwd = 2, cex.axis=1.5,
      cex.lab = 1.5)
  
  #Plot the Partial Auto-Correlation
  pacf(pcw$x[,1], main = paste0("Partial ACF for Reanalysis Data PC-1 - ",Field_Name),
       xlab = "Lag (Days)",
       xlim = c(0,30), ylim = c(-0.05,1),
       lwd = 2, cex.axis=1.5,
       cex.lab = 1.5)
...
get_seasonal_corr <- function(Fld1, Fld2, Grid, start_date){
  ...
  ###Print the plots
  print(ggarrange(p1, p2, p3, p4, nrow = 2, ncol = 2, common.legend = TRUE, legend = "right"))
...
pdf("figures/Data_Characteristics.pdf")
...
#Get the ACF and PACF of Solar PC-1
get_pca_pacf(Fld = ssrd, Field_Name = "Solar")
